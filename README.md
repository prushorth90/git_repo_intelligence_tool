...
Baseline analysis persists source file count, relevant text size, language distribution, directory structure (up to four levels), file-extension counts, and whether history was included. It skips symlinks, binary formats/content, oversized files, generated/minified artifacts, lock files, and directories such as `.git`, `node_modules`, `target`, `build`, `dist`, `vendor`, `coverage`, `.next`, virtual environments, IDE metadata, and generated output. Dependency, pull-request, and composite-risk analyzers remain future phases.

Supported Java, C#, Python, TypeScript, and TSX files are parsed with Tree-sitter through the `SourceCodeParser` abstraction. Language adapters share one AST walker that extracts class/interface counts, methods and functions, import statements, callable line spans, maximum control-flow nesting, control-flow counts, parse-error state, and compact symbol summaries. Flyway V9 stores this structural metadata, and V10 adds file and callable complexity metrics. Full source contents are never persisted. The worker uses a glibc JRE because the packaged Tree-sitter native grammars target glibc Linux.

Cyclomatic complexity is an AST-derived approximation defined as `1 + decision points`. A decision point is each `if`, loop, `catch`/`except`, ternary conditional, non-default `switch`/`match` case, and direct logical AND or OR operator. The file score includes all decision points in the file. Each method or function is also scored independently; nested callable bodies are excluded from their parent's score to prevent double-counting. Parse-error files do not produce callable rankings. The Complexity page displays the latest analysis's top 100 files and methods/functions, and the API includes these rules with every response.
